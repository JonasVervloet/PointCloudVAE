    ...
    def train(self, end_epoch, lr, weight_decay, start_epoch=0):
        ...
        for i in range(end_epoch - start_epoch + 1):
            ...
            for batch in train_loader:
                optimizer.zero_grad()
                in_points_list, in_batch_list, out_points_list, out_batch_list, latent_features = self.network(batch)
                loss = loss_fn(in_points_list, in_batch_list, out_points_list, out_batch_list)
                loss.backward()
                optimizer.step()
